package com.jsen.test.common.joker;
 ...
 * @since 2018/5/23
 */
public class JokerStaticHandlerImpl implements StaticHandler {

    private static final Logger log = LoggerFactory.getLogger(JokerStaticHandlerImpl.class);

    private final DateFormat dateTimeFormatter = Utils.createRFC1123DateTimeFormatter();
    private Map<String, JokerStaticHandlerImpl.CacheEntry> propsCache;
    ...
    public static String getJokerRoot(Class clazz) {
        ...
        if (libDir.getName().endsWith(".jar")) {
            filePath = libDir.getName();
            if (filePath.contains(".")) {
                filePath = filePath.substring(0, filePath.lastIndexOf("."));
            }
            return new File(new File(new File(root, "static"), filePath), "webroot").getAbsolutePath();
        } else {
            return new File(libDir, "webroot").getAbsolutePath();
        }
    }
    ...
    private void sendStatic(RoutingContext context, String path) {
        ...
        // Look in cache
        JokerStaticHandlerImpl.CacheEntry entry;
        if (cachingEnabled) {
            ...
            getFileProps(context, sfile, res -> {
                if (res.succeeded()) {
                    ...
                    if (fprops == null) {
                        ...
                        propsCache().put(path, new JokerStaticHandlerImpl.CacheEntry(fprops, System.currentTimeMillis()));
